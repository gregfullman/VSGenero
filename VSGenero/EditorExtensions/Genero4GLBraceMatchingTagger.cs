...
namespace VSGenero.EditorExtensions
{
    ...
    internal class BraceMatchingTaggerProvider : IViewTaggerProvider
    {
        public ITagger<T> CreateTagger<T>(ITextView textView, ITextBuffer buffer) where T : ITag
        {
            ...
            return new Informix4GLBraceMatchingTagger(textView, buffer) as ITagger<T>;
        }
    }

    internal class Informix4GLBraceMatchingTagger : ITagger<TextMarkerTag>
    {
        ...
        internal Informix4GLBraceMatchingTagger(ITextView view, ITextBuffer sourceBuffer)
        {
            //here the keys are the open braces, and the values are the close braces
            m_braceList = new Dictionary<char, char>();
            // TODO: read these from the XML config file
            m_braceList.Add('{', '}');
            m_braceList.Add('[', ']');
            m_braceList.Add('(', ')');
            this.View = view;
        ...
        public IEnumerable<ITagSpan<TextMarkerTag>> GetTags(NormalizedSnapshotSpanCollection spans)
        {
            ...
            if (m_braceList.ContainsKey(currentText))   //the key is the open brace
            {
                ...
                if (Informix4GLBraceMatchingTagger.FindMatchingCloseChar(currentChar, currentText, closeChar, View.TextViewLines.Count, out pairSpan) == true)
                {
            ...
            else if (m_braceList.ContainsValue(lastText))    //the value is the close brace, which is the *previous* character 
            {
                var open = from n in m_braceList
                           ...
                           select n.Key;
                if (Informix4GLBraceMatchingTagger.FindMatchingOpenChar(lastChar, (char)open.ElementAt<char>(0), lastText, View.TextViewLines.Count, out pairSpan) == true)
                {
