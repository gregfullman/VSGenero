...
namespace VSGenero.EditorExtensions
{
    public class Genero4glClassifier : IClassifier, IDisposable
    {
        ...
        public IList<TokenInfo> GetTokens(SnapshotSpan span, bool useCache = true)
        {
            ...
            if (span.Length > 0)
            {
                AddTokens(GetTokenizer(false), tokens, span, useCache);
            }
        ...
        private void AddTokens(Tokenizer tokenizer, List<TokenInfo> tokens, SnapshotSpan span, bool useCache = true)
        {
            ...
            {
                if (!useCache || !_tokenCache.TryGetTokenization(currentLine, out lineTokenization))
                {
