...
namespace VSGenero.Snippets
{
    class ExpansionClient : IVsExpansionClient
    {
        ...
        public int FormatSpan(IVsTextLines pBuffer, TextSpan[] ts)
        {
            ...
            if (ErrorHandler.Failed(_session.GetSnippetNode("CodeSnippet:Code", out codeNode)) || codeNode == null)
            {
                return hr;
            }

            if (ErrorHandler.Failed(_session.GetHeaderNode("CodeSnippet:SnippetTypes", out snippetTypes)) || snippetTypes == null)
            {
                return hr;
            }
